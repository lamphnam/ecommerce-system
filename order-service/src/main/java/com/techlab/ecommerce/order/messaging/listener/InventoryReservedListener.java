...
public class InventoryReservedListener {
    ...
    @RabbitListener(queues = QueueNames.ORDER_INVENTORY_RESERVED)
    public void onMessage(EventEnvelope<InventoryReservedPayload> event,
                          Channel channel,
                          @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag) throws IOException {
        log.debug("Received {} eventId={}", RoutingKeys.INVENTORY_RESERVED, event.getEventId());
        ackHandler.handle(event, channel, deliveryTag, RoutingKeys.INVENTORY_RESERVED,
                sagaService::handleInventoryReserved);
    }
}
