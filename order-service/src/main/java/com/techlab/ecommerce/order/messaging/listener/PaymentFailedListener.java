...
public class PaymentFailedListener {
    ...
    @RabbitListener(queues = QueueNames.ORDER_PAYMENT_FAILED)
    public void onMessage(EventEnvelope<PaymentFailedPayload> event,
                          Channel channel,
                          @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag) throws IOException {
        log.debug("Received {} eventId={}", RoutingKeys.PAYMENT_FAILED, event.getEventId());
        ackHandler.handle(event, channel, deliveryTag, RoutingKeys.PAYMENT_FAILED,
                sagaService::handlePaymentFailed);
    }
}
