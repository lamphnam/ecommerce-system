...
export function login(email, password) {
    ...
    // ApiResponse wraps the token in .data or .data.token depending on auth impl
    return body.data?.token || body.data || null;
}
...
export function rabbitQueueDepth(queueName) {
    const res = http.get(`${RABBITMQ_API}/queues/%2F/${queueName}`, {
        headers: { 'Authorization': 'Basic ' + encoding.b64encode('techlab:techlab') },
    });
